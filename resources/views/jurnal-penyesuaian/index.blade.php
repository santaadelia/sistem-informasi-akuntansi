@extends('layouts.app')
@section('judul','Jurnal Penyesuaian - Sistem Informasi Akuntansi')
    ...
    <div class="card shadow">
        <div class="card-body">
            <div class="table-responsive">
                        ...
                        @forelse ($jurnal_penyesuaian as $item)
                            <tr>
                                <td style="vertical-align: middle">
                                    <input type="checkbox" class="jurnal-penyesuaian-checkbox" id="delete{{ $item->id }}" name="delete[]" value="{{ $item->id }}">
                                </td>
                                <td style="vertical-align: middle">
                ...
                {{ $jurnal_penyesuaian->links('layouts.footers.pagination') }}
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body pt-0">
                <form class="d-inline" action="{{ route("jurnal-penyesuaian.laporan") }}" method="get">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '#delete', function(){
            ...
            if (confirm("Apakah anda yakin ingin menghapus data ini?")) {
                ...
                if (id.length > 0) {
                    $.ajax({
                        url     : "{{ route('jurnal-penyesuaian.destroys') }}",
                        method  : 'delete',
                        data    : {
                            _token  : "{{ csrf_token() }}",
                            id      : id,
